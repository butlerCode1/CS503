...
int open_db(char *dbFile, bool should_truncate){
    ...
    mode_t mode = S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP; //The constants at the end of the variable are file permission macros defined
                                                                                                        //Within the <sys/stat.h> file. They are commonly brough up when altering or creating file permissions
                                                                                                        //In Lunux-based systems.

    //open the file if it exists for Read and Write, 
    //create it if it does not exist
    int    flags = O_RDWR | O_CREAT;

    if (should_truncate) // In the case that should_truncate is true
        flags += O_TRUNC; // Add the O_TRUNC constant to the flags varibale capabilities. The O_TRUNC constant clears the file contents to 0 bytes
...
int get_student(int fd, int id, student_t *s){
    ...
    // int id = id * sizeof(student_t);// Initialization of Id and gets the size of the student record in bytes
...
int add_student(int fd, int id, char *fname, char *lname, int gpa){
    ...
    student_t mystudent;
    memset(&mystudent, 0, sizeof(student_t));
    student_t myStudentEmpty = {0};
    ...
    // printf("Need to add student id %d at offset %ld\n", id, offset);
     ...
     if (read(fd, &mystudent, sizeof(student_t)) != sizeof(student_t)) {
         return M_ERR_DB_READ;
    }
    ...
    ssize_t write_bytes = write(fd, &mystudent, sizeof(student_t));
    if(write_bytes != sizeof(student_t)) {
    ...
    // return    printf("Need to add student id %d at offset %ld\n", id, offset);
    ...
    // //if yes print dup error msg and return dp op error
    // //if no, no erro continue on with adding student

    // // to do what offset should this record be stored at?
    ...
    // //TODO - write the record to the db file - emember the fd for db is in arg "fd"

    // //to do determine if the write was a success, print the correct messages, and return the correct coode
...
int main(int argc, char *argv[]){
    ...
    switch(opt){
        ...
        case 'f':
            ...
            return get_student(fd,id, &student);
